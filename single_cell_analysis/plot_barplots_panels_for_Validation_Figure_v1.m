...
glut_list = fliplr({'Maf','Cpne4','Cck','Trh','Nts','Tac2','Sst','Nmu','Nmur2','Npff','Reln','Tac1','Lamp5','Qrfpr','Ecel1','Meis2','Elavl4','Lypd1','Slc17a6'});
[~,loclist] = ismember(glut_list, geneuni_1);
glut_list(loclist==0) = [];
loclist(loclist==0) = [];

% yax_lim = [0,space_gr*length(T_cells_tmp_uni) + length(moldata_validcells(1,:))];
yax_lim = [0,space_gr*14 + length(moldata_validcells(1,1:max(find(T_cells_tmp==15))))];
hf = figure('color','w','position',[100,100,1450,400],'visible','on');
[ha, pos] = tight_subplot(19, 1, [0.01,0.01], [0.01,0.01], [0.01,0.01]);
tic
for k=1:length(loclist)
    k
    axes(ha(k))
    hold on;
    maxval = 0.8*max(moldata_validcells(loclist(k),:));
    if maxval>0
        for i=1:15;%length(T_cells_tmp_uni)
            ind = find(T_cells_tmp==i);                    
            plot((ind(1) + (i-1)*space_gr-1)*[1,1],[0,maxval], '--','color',220/255*[1,1,1]);
            plot((ind(end) + (i-1)*space_gr+0.5)*[1,1],[0,maxval], '--','color',220/255*[1,1,1]);
            h = bar(ind + (i-1)*space_gr,moldata_validcells(loclist(k),ind));
...
%     set(gca,'xtick',gr_cent,'xticklabel',tree_order_label,'fontsize',14,'ylim',[0,maxval],'xlim',yax_lim,'xdir','normal','XTickLabelRotation',45)
    set(gca,'xtick',[],'fontsize',14,'ylim'....
        ,[0,maxval],'xlim',yax_lim,'xdir','normal')
    ylabel(glut_list{k},'fontsize',6);

    set(gca,'xlim',yax_lim);
end

eval(['export_fig glut_barplots_valiFig_15x1_',date,'.pdf'])


% % % % % % % % % % % % % % % % % % % 


gaba_list = fliplr({'Gal','Slc17a6','Pnoc','Rspo3','Tac2','Gad2','Npy','Qrfpr','Ecel1','Crabp1','Calb2','Tac1','Gad1','Adamts5','Cplx1','Sorcs3','Sst','Spp1'});
[~,loclist] = ismember(gaba_list, geneuni_1);
gaba_list(loclist==0) = [];
loclist(loclist==0) = [];


yax_lim = [space_gr*15 + length(moldata_validcells(1,1:max(find(T_cells_tmp==15))))....
    ,space_gr*29 + length(moldata_validcells(1,1:max(find(T_cells_tmp==30))))];
hf = figure('color','w','position',[100,100,1450,400],'visible','on');
[ha, pos] = tight_subplot(18, 1, [0.01,0.01], [0.01,0.01], [0.01,0.01]);
tic
for k=1:length(loclist)
    k
    axes(ha(k))
    ...
    maxval = 0.8*max(moldata_validcells(loclist(k),:));
    if maxval>0
        for i=16:30;%length(T_cells_tmp_uni)
            ind = find(T_cells_tmp==i);                    
            plot((ind(1) + (i-1)*space_gr-1)*[1,1],[0,maxval], '--','color',220/255*[1,1,1]);
            plot((ind(end) + (i-1)*space_gr+0.5)*[1,1],[0,maxval], '--','color',220/255*[1,1,1]);
            h = bar(ind + (i-1)*space_gr,moldata_validcells(loclist(k),ind));
...
%     set(gca,'xtick',gr_cent,'xticklabel',tree_order_label,'fontsize',14,'ylim',[0,maxval],'xlim',yax_lim,'xdir','normal','XTickLabelRotation',45)
    set(gca,'xtick',[],'fontsize',14,'ylim'....
        ,[0,maxval],'xlim',yax_lim,'xdir','normal')
    ylabel(gaba_list{k},'fontsize',6);

    set(gca,'xlim',yax_lim);
end

eval(['export_fig gaba_barplots_valiFig_15x1_',date,'.pdf'])
