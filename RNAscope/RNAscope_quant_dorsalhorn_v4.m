function [] = RNAscope_quant_dorsalhorn_v4(path_folder,filename,chan1name,chan2name,chan3name,chan4name,manual_th,fig_flag,enhan_prct,p_maxtranf)
...
% chan4name = 'fos';
% manual_th = 2;
if strfind(filename,'LSC')
    load ref_LSC;
    ref_I = ref_LSC;
    ref_set = set_ref_LSC;
    clear ref_LSC set_ref_LSC
elseif strfind(filename,'CSC')
    load ref_CSC;
    ref_I = ref_CSC;
    ref_set = set_ref_CSC;
    clear ref_CSC set_ref_CSC
end
f = dir([path_folder,filename(1:end-4),'_ref_points*txt']);
if length(f)>1
    [~,imax] = max(cellfun(@datenum, {f.date}'));
    f = f(imax);
end
pointset = cell2mat(loadCellFile([path_folder,f.name]));
pointset = round([pointset(1:5,1:2);pointset(1:5,3:4)]);

nbit = 16;
dapi = imread([path_folder,filename],'index',3);
dapi = imrotate(dapi,-90);

im_range = [max([min(pointset(:,1))-100,1]), min([max(pointset(:,1))+100,length(dapi(1,:))]);...
    max([min(pointset(:,2))-100,1]), min([max(pointset(:,2))+100,length(dapi(:,1))])];
pointset = [pointset(:,1)-im_range(1,1), pointset(:,2)-im_range(2,1)];
tform = fitgeotrans(pointset,ref_set,'affine');

% nbit = 16;
% dapi = imread([path_folder,filename],'index',3);
% dapi = imrotate(dapi,-90);
dapi = dapi(im_range(2,1):im_range(2,2),im_range(1,1):im_range(1,2));
dapi = imwarp(dapi,tform,'OutputView',imref2d(size(ref_I)));
...
figure('visible',fig_flag); imshowpair(dapi,ref_I);hold on;
plot(ref_set(:,1),ref_set(:,2),'+y','markersize',20);
plot(ref_set(:,1),ref_set(:,2),'or','markersize',20);
eval(['export_fig ',path_folder,filename(1:end-4),'_dapi_refpoints_',date,'.pdf']);
...
geneC1 = imadjust(geneC1,[0,double(prctile(geneC1(:),enhan_prct(1)))/(2^nbit-1)],[0,1]);
...
geneC1 = geneC1(im_range(2,1):im_range(2,2),im_range(1,1):im_range(1,2));
geneC1 = imwarp(geneC1,tform,'OutputView',imref2d(size(ref_I)));
...
geneC2 = imadjust(geneC2,[0,double(prctile(geneC2(:),enhan_prct(2)))/(2^nbit-1)],[0,1]);
...
geneC2 = geneC2(im_range(2,1):im_range(2,2),im_range(1,1):im_range(1,2));
geneC2 = imwarp(geneC2,tform,'OutputView',imref2d(size(ref_I)));
...
geneC4 = imadjust(geneC4,[0,double(prctile(geneC4(:),enhan_prct(3)))/(2^nbit-1)],[0,1]);
...
geneC4 = geneC4(im_range(2,1):im_range(2,2),im_range(1,1):im_range(1,2));
geneC4 = imwarp(geneC4,tform,'OutputView',imref2d(size(ref_I)));
% % % % % % % % % % % % % % % % % % % % % % %
...
if strcmpi(fig_flag,'on'); figure('visible',fig_flag); imshow(rgb_bw); hold on; end
...
if strcmpi(fig_flag,'on'); figure('visible',fig_flag); imshow(rgb_bw_w); hold on; end
...
if isempty(dir([path_folder,filename(1:end-4),'_dapi_segmentation.mat']))
    ...
    if sum(size(bw3)==size(ref_I))<2
        bw3 = bw3(im_range(2,1):im_range(2,2),im_range(1,1):im_range(1,2));
        bw3 = imwarp(bw3,tform,'OutputView',imref2d(size(ref_I)));
    end
    stats_dapi = regionprops(bw3,'area','boundingbox','PixelList','PixelIdxList');
    dapi_a = [stats_dapi.Area];
    bb_bw1 = [reshape([stats_dapi.BoundingBox]',4,length(stats_dapi))]';
    for k=1:length(dapi_a)
        k
...
geneC1_cell = zeros(max(L(:)),1);
geneC4_cell = zeros(max(L(:)),1);
geneC2_cell = zeros(max(L(:)),1);
geneC1_dots_cell = zeros(max(L(:)),1);
geneC4_dots_cell = zeros(max(L(:)),1);
geneC2_dots_cell = zeros(max(L(:)),1);
area_cell = zeros(max(L(:)),1);
%     figure; imagesc(L);
for i=1:max(L(:));
    i
...
% % % % % % % % % % % % % % % %
table1 = [ {'dapi_x','dapi_y','area',chan1name,chan2name,chan4name};m2c([dapi_cent,area_cell,geneC1_dots_all,geneC2_dots_all,geneC4_dots_all]) ];
saveCellFile(table1,[path_folder,filename(1:end-4),'_position_counts_per_dapi_',date,'.txt']);
